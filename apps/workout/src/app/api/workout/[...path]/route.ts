...
import {isSaveLog} from "../../../../model/tracking";
...
async function proxy(request: Request, context: {params: Promise<{path: string[]}>}) {
    ...
    const validRoute = read ? publicRead || route === "plans" || /^(plans|history)\/[0-9a-f-]{36}$/i.test(route)
        : request.method === "POST" ? route === "plans" || route === "logs"
        : request.method === "PUT" && /^plans\/[0-9a-f-]{36}$/i.test(route);
    if (!validRoute) return Response.json({code: "notFound"}, {status: 404});
    ...
    try {
        ...
        if (!read) {
            ...
            if (!(route === "logs" ? isSaveLog(body) : isPlan(body))) return Response.json({code: "invalidPlan"}, {status: 400});
        }
        const query = new URL(request.url).searchParams;
        const page = query.get("page") ?? "0";
        if (!/^\d{1,5}$/.test(page) || Number(page) > 10000) return Response.json({code: "invalidPlan"}, {status: 400});
        const response = await fetch(`${process.env.AUTH_BACKEND_URL ?? "http://127.0.0.1:10081"}/internal/workout/${route}${(route === "plans" || route.startsWith("history/")) && read ? `?page=${page}` : ""}`, {
