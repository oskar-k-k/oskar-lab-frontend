...
describe("actual workout results", () => {
    it("records actual reps and holds without forcing target ranges", () => {
        expect(completedSets([{value: "12", weight: ""}, {value: "0", weight: ""}], "reps"))
    ...
    it("keeps set numbers when skipping blank rows and accepts decimal weights", () => {
        expect(completedSets([{value: "", weight: ""}, {value: "8", weight: "12,5"}], "weighted"))
            .toEqual([{setNumber: 2, value: 8, weight: 12.5}]);
        expect(completedSets([{value: "8", weight: "0"}], "weighted")).not.toBeNull();
    });
    it("rejects incomplete, fractional, negative and empty sets", () => {
        for (const row of [{value: "8", weight: ""}, {value: "", weight: "12"}, {value: "1.5", weight: "12"},
            {value: "-1", weight: "12"}, {value: "8", weight: "1.001"}, {value: "", weight: ""}])
            expect(completedSets([row], "weighted")).toBeNull();
    });
    it("validates private write payloads and historic units", () => {
        const log = {id, planId: id, planVersion: 0, exerciseId: id, position: 0, trackingMode: "seconds", sets: [{setNumber: 1, value: 15, weight: null}]};
        expect(isSaveLog(log)).toBe(true);
        expect(isSaveLog({...log, sets: [...log.sets, ...log.sets]})).toBe(false);
        expect(isSaveLog({...log, trackingMode: "weighted"})).toBe(false);
        expect(isSaveLog({...log, position: -1})).toBe(false);
        expect(isExerciseLog({...log, planName: "A", recordedAt: "2026-09-21T10:00:00Z"})).toBe(true);
        expect(isExerciseLog({...log, planName: "A", recordedAt: "invalid"})).toBe(false);
    });
});
