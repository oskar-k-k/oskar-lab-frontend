...
function parseSource(filePath: string, source: string): SourceDocumentation {
    ...
    return {path: path.relative(path.resolve(process.cwd(), "../.."), filePath).replaceAll("\\", "/"), symbols};
}
...
export function generateComponentDocumentation(): Promise<SourceDocumentation[]> {
    return Promise.all(["ui", "platform-shell"].map(name => generateSourceDocumentation(path.resolve(process.cwd(), `../../packages/${name}/src`)))).then(groups => groups.flat());
}
